...
// Temporary guard: auth() doesn't redirect yet, so check the session ourselves.
...
// Always pull a fresh copy of the row so numTel (not stored in session) and
// any previous edits are reflected.
...
// Helper for mysqli_stmt_bind_param with a variable number of fields.
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (($_POST["action"] ?? "") == "change_password") {
		...
		if (!$currentPassword || !$newPassword || !$confirmPassword) {
			echo json_encode(["success" => false, "field" => "current", "message" => "Please fill in all password fields."]);
			exit;
		}

		$strongPassword = '/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/';
		if (!preg_match($strongPassword, $newPassword)) {
			echo json_encode(["success" => false, "field" => "new", "message" => "Password does not meet the requirements."]);
			exit;
		}

		if ($newPassword !== $confirmPassword) {
			echo json_encode(["success" => false, "field" => "confirm", "message" => "New password and confirm password do not match."]);
			exit;
		}

		// Strict one-way check only — no plain-text comparison.
		if (!password_verify($currentPassword, $user["password"])) {
			echo json_encode(["success" => false, "field" => "current", "message" => "Current password is incorrect."]);
			exit;
		}
...
<body>
	...
	<script>
		...
		// Now actually verifies + saves through this same page (action=change_password)
		...
		// Now saves email/phone/avatar through this same page (action=update_profile)
		function saveChanges() {
			...
			if (file) {
				...
				reader.onload = function(e) {
					sendUpdate(e.target.result); // base64 data URI, fits the LONGTEXT column
				};
